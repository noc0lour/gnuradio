...
ofdm_insert_preamble::make(int fft_length, const std::vector<std::vector<gr_complex>>& preamble) {
    return gnuradio::get_initial_sptr(new ofdm_insert_preamble_impl(fft_length, preamble));
}
...
ofdm_insert_preamble_impl::ofdm_insert_preamble_impl(
    ...
    : block("ofdm_insert_preamble",
            ...
            io_signature::make2(1, 2, sizeof(gr_complex) * fft_length, sizeof(char))),
      d_fft_length(fft_length), d_state(ST_IDLE), d_nsymbols_output(0), d_pending_flag(0),
      d_preamble(preamble) {
...
void ofdm_insert_preamble_impl::forecast(int noutput_items, gr_vector_int& ninput_items_required) {
...
int ofdm_insert_preamble_impl::general_work(int noutput_items,
    ...
    int ninput_items = ninput_items_v.size() == 2 ? std::min(ninput_items_v[0], ninput_items_v[1])
                                                  : ninput_items_v[0];
