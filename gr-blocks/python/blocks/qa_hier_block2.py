...
class test_hier_block2(gr_unittest.TestCase):
    ...
    def test_005_connect_output_in_use(self):
        ...
        hblock.connect(nop1, hblock)
        self.assertRaises(RuntimeError, lambda: hblock.connect(nop2, hblock))
    ...
    def test_022_connect_single_with_ports(self):
        ...
        self.assertRaises(RuntimeError, lambda: hblock.connect(blk))
    ...
    def test_025_disconnect_single_not_connected(self):
        ...
        blk = gr.hier_block2("block", gr.io_signature(0, 0, 0),
                             gr.io_signature(0, 0, 0))
        self.assertRaises(RuntimeError, lambda: hblock.disconnect(blk))

    def test_026_run_single(self):
        expected_data = (1.0, )
        tb = gr.top_block("top_block")
        hb = gr.hier_block2("block", gr.io_signature(0, 0, 0),
                            gr.io_signature(0, 0, 0))
    ...
    def test_027d_fully_unconnected_input(self):
        ...
        hb = gr.hier_block2("block", gr.io_signature(1, 1, 1),
                            gr.io_signature(1, 1, 1))
        hdst = blocks.vector_source_b([
            1,
        ])
        hb.connect(hdst, hb)  # wire output internally
        dst = blocks.vector_sink_b()
        tb.connect(hb,
                   dst)  # hb's input is not connected internally or externally
        self.assertRaises(RuntimeError, lambda: tb.run())
