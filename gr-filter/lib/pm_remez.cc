...
static void calc_parms(
    int r, int Ext[], double Grid[], double D[], double W[], double ad[], double x[], double y[]) {
    int i, j, k, ld;
...
static int remez(double h[],
     ...
     */
    create_dense_grid(
        r, numtaps, numband, bands, des, weight, gridsize, Grid, D, W, symmetry, griddensity);
...
std::vector<double> pm_remez(int order,
    ...
    int err = remez(coeff, numtaps, numbands, bands, response, weight, itype, grid_density);
