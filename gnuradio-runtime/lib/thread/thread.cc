...
static void _set_thread_name(gr_thread_t thread, const char* name, DWORD dwThreadId) {
    ...
    __try {
        RaiseException(
            SET_THREAD_NAME_EXCEPTION, 0, sizeof(info) / sizeof(ULONG_PTR), (ULONG_PTR*)&info);
...
#else
void set_thread_name(gr_thread_t thread, std::string name) { /* Not implemented on mingw-w64 */ }
#endif /* !__MINGW32__ */
...
void set_thread_name(gr_thread_t thread, std::string name) {
    ...
    if ((int)name.size() >
        max_len) // Shorten the name if necessary by taking as many characters from the front
