...
class tpb_container {
    ...
    void operator()() { tpb_thread_body body(d_block, d_start_sync, d_max_noutput_items); }
};
...
scheduler_tpb::scheduler_tpb(flat_flowgraph_sptr ffg, int max_noutput_items)
    ...
    thread::barrier_sptr start_sync = boost::make_shared<thread::barrier>(blocks.size() + 1);
